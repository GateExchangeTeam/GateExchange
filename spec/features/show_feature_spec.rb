...
RSpec.describe 'show feature', type: :feature do
  ...
  before :each do
    ...
    Course.create!(title: 'Geography of Happiness', course_code: '350', description: 'Be happy', department: 'GEOL',
                   faculty: 'Joyce')
    @juser = User.create!(email: 'jzhang4@colgate.edu', password: 'testtest')
    p1 = Post.create!(title: 'how to do homework 1',
                 description: 'I have the following file /app/validators/hex_color.rb in my Rails app but I got an error', view: 0, course_id: 1, user: @juser)
    p2 = Post.create!(title: 'grading', description: 'the professor is a easy grader', view: 0, course_id: 1, user: @juser)
    p1.comments.create!(text_body: 'i also have question on hw1', user: @juser)
    p2.comments.create!(text_body: 'no, not a easy grader', user: @juser)
